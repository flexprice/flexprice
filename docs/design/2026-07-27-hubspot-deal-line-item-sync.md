...
- **What ships:** Today, FlexPrice pushes a subscription's flat-rate line items into a HubSpot Deal exactly once, at subscription creation. Nothing re-syncs afterward. This design extends that one-shot sync into an ongoing, event-driven sync: whenever a `FIXED` line item is added, its quantity/price changes, it ends, or the subscription is cancelled, the corresponding HubSpot Deal line item is created, updated in place, or deleted to match — and the Deal's `amount` is re-derived from HubSpot's own recalculated ACV afterward, same as today.
...
- **New infra:** No new Temporal workflow type — the existing `HubSpotDealSyncWorkflow` (`TemporalHubSpotDealSyncWorkflow`) is extended in place to take a single mutated line item and an operation (`created` / `updated` / `deleted`) instead of always syncing the full line item set, fired fire-and-forget from five subscription-service call sites.
...
This design closes that gap: every FlexPrice-side mutation to a `FIXED` line item on a HubSpot-linked subscription pushes a corresponding create/update/delete to HubSpot, keeping the Deal's line items — and therefore its ACV-derived amount — continuously accurate.
...
- Every `FIXED`/active line item mutation on a HubSpot-linked subscription (add, quantity/price change, end, addon add/remove, subscription cancellation) results in the matching HubSpot Deal line item being created, updated, or deleted — without requiring a full resync of every line item on the Deal each time.
- Prefer updating existing HubSpot line items in place over delete-and-recreate, per HubSpot's own guidance that recreating breaks reporting/forecast continuity on the Deal.
...
| **Deal line item** | A HubSpot object representing one priced item on a Deal (`hs_line_item`), with `quantity`, `price`, `amount`, `hs_recurring_billing_period`. Created/updated/deleted via the HubSpot Line Items API. |
...
sequenceDiagram
    ...
    API->>Svc: UpdateSubscriptionLineItem(quantity: 5)
    Svc->>Svc: commit DB change
    Svc-->>WF: fire-and-forget (line_item_id, operation=updated)
    WF->>EIM: lookup mapping(subscription_line_item, line_item_id)
    EIM-->>WF: hubspot_line_item_id
    WF->>HS: PATCH /line_items/{id} { quantity: 5 }
    WF->>WF: sleep 10s
    WF->>HS: GET /deals/{deal_id} (read hs_acv)
    WF->>HS: PATCH /deals/{deal_id} { amount: acv }
```
...
- `DealSyncService` gains `UpdateHubSpotLineItem` and `DeleteHubSpotLineItem` methods alongside the existing `createHubSpotLineItem`, all operating against a single line item rather than the full set.
...
Written on `created`, read on `updated`/`deleted`, deleted on `deleted` (or self-healed away on a 404 from HubSpot — see §9).

## 8. Approach

### 8.1 Trigger call sites in `subscriptionService`

All five fire the workflow fire-and-forget, after the DB transaction commits — same pattern as today's `triggerHubSpotDealSyncWorkflow` call. Each skips silently (no workflow fired) if `resolveDealID` finds nothing, or the mutated line item is not `FIXED`+active.

| Call site | Mutation | Operation |
|---|---|---|
| `AddSubscriptionLineItem` | new line item | `created` |
| `UpdateSubscriptionLineItem` | quantity/price change | `updated` |
| `DeleteSubscriptionLineItem` | line item ended | `deleted` |
...
1. **Create/Update/Delete** — per `operation`:
   - `created`: if a mapping row already exists for this line item (retry-safety), treat as `updated` instead of double-creating. Otherwise `POST /line_items`, associate to the deal, persist the mapping.
   - `updated`: look up the mapping; `PATCH /line_items/{id}` with new quantity/price/amount/description. If HubSpot returns 404 (line item deleted on HubSpot's side), log a warning, delete the stale local mapping, and fall through to `created` behavior instead of failing.
   - `deleted`: look up the mapping; `DELETE /line_items/{id}`; delete the mapping row. A 404 here is treated as already-done (no-op), not a failure.
...
- **Idempotent create**: an existing mapping row for the line item short-circuits `created` into an update, so a retried "created" workflow never double-creates.
- **Self-healing on drift**: a 404 from HubSpot on `PATCH`/`DELETE` (deal or line item removed on HubSpot's side) deletes the stale local mapping rather than failing the workflow — the next mutation on that line item creates fresh, rather than requiring a reconciliation pass to notice the mapping is stale.
...
1. Add `subscription_line_item` to `IntegrationEntityType` (+ validation).
2. Extend `DealSyncService` with per-line-item create/update/delete methods and the mapping-backed `resolveDealID`.
3. Extend `HubSpotDealSyncWorkflow` input/branching in place (no Temporal registration changes needed, since the workflow type name is unchanged).
4. Wire the five trigger call sites in `subscriptionService`.
5. Backfill happens automatically and lazily (§7.1, step 2) — no explicit migration script needed; existing HubSpot-linked subscriptions get their `entity_integration_mapping` row the first time any line item on them changes after this ships.
6. No config/flag rollout needed — ships gated on the existing `Deal.Outbound` flag, so tenants already opted into Deal sync get the new behavior automatically.
...
| Update strategy | Update-in-place; delete on end | HubSpot's own guidance: delete-and-recreate breaks Deal reporting/forecast continuity. |
