...
**Linear:** [FLE-974](https://linear.app/flexprice/issue/FLE-974)  
...
7. **Credit grants:** extend subscription-scoped grants whose `EndDate` equaled the old subscription end to the new end (optional but recommended for consistency with create-time binding).
...
- Grants with `EndDate == old_end`: update to `new_end`.
...
- [ ] DTO: `end_date` type + `SubModifyEndDateRequest` + request validate switch
- [ ] Service: `subscription_modification_end_date.go` (validate / preview / execute / cascade)
- [ ] Dispatch wiring in `Execute` / `Preview`
- [ ] Line-item selector + tests for override / onetime / qty-change / phase cases
- [ ] Cancellation unwind path + tests
- [ ] Period-end recompute when clipped
- [ ] Credit grant extend
- [ ] Swagger annotations + `make swagger`
- [ ] Flow doc blurb in `docs/FLOWS/subscription-lifecycle.md`
