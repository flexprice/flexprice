...
func (cc *ConnectionCreate) SetNillableName(s *string) *ConnectionCreate {
	if s != nil {
		cc.SetName(*s)
	}
	return cc
...
func (cc *ConnectionCreate) check() error {
	...
	if _, ok := cc.mutation.SecretID(); !ok {
		return &ValidationError{Name: "secret_id", err: errors.New(`ent: missing required field "Connection.secret_id"`)}
	}
	if v, ok := cc.mutation.SecretID(); ok {
		if err := connection.SecretIDValidator(v); err != nil {
			return &ValidationError{Name: "secret_id", err: fmt.Errorf(`ent: validator failed for field "Connection.secret_id": %w`, err)}
		}
	}
	return nil
}
