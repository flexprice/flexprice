...
func (e *Entitlement) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case entitlement.FieldAddonID:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field addon_id", values[i])
			} else if value.Valid {
				e.AddonID = new(string)
				*e.AddonID = value.String
			}
		...
		case entitlement.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field plan_entitlements", values[i])
			} else if value.Valid {
				e.plan_entitlements = new(string)
				*e.plan_entitlements = value.String
			}
		default:
