...
func (i *InvoiceLineItem) Validate() error {
	...
	if i.DiscountApplied.IsNegative() {
		return ierr.NewError("invoice line item validation failed").WithHint("discount_applied must be non negative").Mark(ierr.ErrValidation)
	}
