...
func (s *BillingServiceSuite) setupSubWithFixedLineItemsForPeriodTests(
	...
	firstPeriodEnd, err := types.NextBillingDate(context.Background(), types.NextBillingDateParams{
...
func nextPeriodsForSub(refStart time.Time, subBillingPeriod types.BillingPeriod, n int) []periodWindow {
	...
	start := refStart
	for i := 0; i < n; i++ {
		end, err := types.NextBillingDate(context.Background(), types.NextBillingDateParams{
