	...
	h.processors = map[types.WebhookEventName]eventProcessor{
		...
		types.WebhookEventSubscriptionLineItemCreated: func(ctx context.Context, event *types.WebhookEvent, msg *message.Message) error {
			return DispatchHubSpotDealLineItemSync(
				ctx, h.deps.Config, h.deps.ConnectionRepo, h.deps.CustomerRepo, h.deps.EIMRepo,
				h.deps.Logger, event, msg.UUID, types.WebhookEventSubscriptionLineItemCreated,
			)
		},
		types.WebhookEventSubscriptionLineItemDeleted: func(ctx context.Context, event *types.WebhookEvent, msg *message.Message) error {
			return DispatchHubSpotDealLineItemSync(
				ctx, h.deps.Config, h.deps.ConnectionRepo, h.deps.CustomerRepo, h.deps.EIMRepo,
				h.deps.Logger, event, msg.UUID, types.WebhookEventSubscriptionLineItemDeleted,
			)
		},
	}
	return h
