...
func TestPublish_SecondaryOnly_Success(t *testing.T) {
	secondary := &fakePublisher{}
	ep := newPub(logger.NewNoopLogger(), nil, secondary)

	if err := ep.Publish(context.Background(), sampleEvent()); err != nil {
		t.Fatalf("expected nil error, got %v", err)
	}
	if secondary.callCount() != 1 {
		t.Fatalf("expected one secondary publish, got %d", secondary.callCount())
	}
}
