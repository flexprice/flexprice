...
func (s *billingService) GetCustomerEntitlements(ctx context.Context, customerID string, req *dto.GetCustomerEntitlementsRequest) (*dto.CustomerEntitlementsResponse, error) {
	...
	for _, sub := range subscriptions {
		...
		if err != nil {
			s.Logger.Warn(ctx, "failed to get subscription entitlements, skipping",
				"subscription_id", sub.ID,
				"error", err)
			continue
		}
...
func (s *billingService) GetCustomerUsageSummary(ctx context.Context, customerID string, req *dto.GetCustomerUsageSummaryRequest) (*dto.CustomerUsageSummaryResponse, error) {
	...
	for _, subscriptionID := range subscriptionIDs {
		...
		if err != nil {
			s.Logger.Warn(ctx, "failed to get subscription", "subscription_id", subscriptionID, "error", err)
			continue
		}
	...
	for _, subscriptionID := range subscriptionIDs {
		...
		if err != nil {
			s.Logger.Warn(ctx, "failed to get usage for subscription", "subscription_id", subscriptionID, "error", err)
			continue
		}
		...
		for _, charge := range usage.Charges {
			if featureID, ok := meterFeatureMap[charge.MeterID]; ok {
				...
				if resetPeriod.String() == sub.BillingPeriod.String() {
					...
					usageResult, err := eventService.GetUsageByMeter(ctx, usageRequest)
					if err != nil {
						s.Logger.Warn(ctx, "failed to get daily usage for feature",
					...
					usageResult, err := eventService.GetUsageByMeter(ctx, usageRequest)
					if err != nil {
						s.Logger.Warn(ctx, "failed to get monthly usage for feature",
							"feature_id", featureID,
							"meter_id", meterID,
							"subscription_id", subscriptionID,
							"error", err)
					...
					if err != nil {
						s.Logger.Warn(ctx, "failed to get total usage for never reset feature",
							"feature_id", featureID,
							"meter_id", meterID,
							"subscription_id", subscriptionID,
							"error", err)
	...
	for _, feature := range entitlements.Features {
		...
		if sub, exists := featureSubscriptionMap[featureID]; exists {
			...
			if err != nil {
				s.Logger.Warn(ctx, "failed to get next usage reset at for feature",
					"feature_id", featureID,
					"subscription_id", sub.ID,
					"error", err)
				continue
