...
func GetCredentialsFunc(secretService SecretService) func(ctx context.Context, provider string) (map[string]string, error) {
	return func(ctx context.Context, provider string) (map[string]string, error) {
...
type gatewayService struct {
	ServiceParams
	gateways         map[string]map[string]integrations.IntegrationGateway // provider -> tenant -> gateway
	gatewayFactories map[string]integrations.GatewayFactory                // provider -> factory
...
type GatewayService interface {
	...
	RegisterGatewayFactory(provider string, factory integrations.GatewayFactory)

	// Get a specific gateway by provider name, initializing it if needed
	GetGateway(ctx context.Context, provider string) (integrations.IntegrationGateway, error)

	// List available providers that have registered factories
	ListAvailableProviders(ctx context.Context) []string
...
func NewGatewayService(
	...
	return &gatewayService{
		...
		gateways:         make(map[string]map[string]integrations.IntegrationGateway),
		gatewayFactories: make(map[string]integrations.GatewayFactory),
		mutex:            sync.RWMutex{},
	}
}

// RegisterGatewayFactory registers a factory function for creating gateways of a specific provider
func (s *gatewayService) RegisterGatewayFactory(provider string, factory integrations.GatewayFactory) {
...
func (s *gatewayService) GetGateway(ctx context.Context, provider string) (integrations.IntegrationGateway, error) {
...
func (s *gatewayService) initializeGateway(ctx context.Context, provider, tenantID string) (integrations.IntegrationGateway, error) {
...
func (s *gatewayService) ListAvailableProviders(ctx context.Context) []string {
	providers := make([]string, 0, len(s.gatewayFactories))
...
func (s *gatewayService) IsProviderSupported(provider string) bool {
...
func (s *gatewayService) ClearCache() {
	s.mutex.Lock()
	defer s.mutex.Unlock()
	s.gateways = make(map[string]map[string]integrations.IntegrationGateway)
