...
func (c *fakeClient) Customers() synthetic.CustomerOps         { return &c.customers }
func (c *fakeClient) Plans() synthetic.PlanOps                 { return &c.plans }
func (c *fakeClient) Prices() synthetic.PriceOps               { return &c.prices }
func (c *fakeClient) Features() synthetic.FeatureOps           { return &c.features }
func (c *fakeClient) Meters() synthetic.MeterOps               { return &c.meters }
func (c *fakeClient) Subscriptions() synthetic.SubscriptionOps { return &c.subs }
func (c *fakeClient) Wallets() synthetic.WalletOps             { return &c.wallets }
func (c *fakeClient) Events() synthetic.EventOps               { return &c.events }
func (c *fakeClient) Invoices() synthetic.InvoiceOps           { return &c.invoices }
func (c *fakeClient) NewAsyncEventClient() synthetic.AsyncEventClient {
	return c.async
}
...
type fakeMeters struct {
	mu      sync.Mutex
	meters  []synthetic.Meter
	created []synthetic.CreateMeterRequest
}

func (f *fakeMeters) List(_ context.Context) ([]synthetic.Meter, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	out := make([]synthetic.Meter, len(f.meters))
	copy(out, f.meters)
	return out, nil
}
func (f *fakeMeters) Create(_ context.Context, req synthetic.CreateMeterRequest) (*synthetic.Meter, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	m := synthetic.Meter{
...
func (f *fakeMeters) Get(_ context.Context, id string) (*synthetic.Meter, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
