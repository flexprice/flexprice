...
func TestMeterHTTPClient_List(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"items":[{"id":"meter_1","event_name":"synthetic_count","name":"X","aggregation":{"type":"COUNT"}}]}`))
	}))
	defer srv.Close()
...
func TestMeterHTTPClient_Create_SendsBody(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(201)
		_, _ = w.Write([]byte(`{"id":"meter_new","event_name":"synthetic_count","name":"X","aggregation":{"type":"COUNT"}}`))
	}))
	defer srv.Close()
	...
	if sent["event_name"] != "synthetic_count" {
