...
func TestLoadConfig_Overrides(t *testing.T) {
	t.Setenv("SYNTHETIC_API_HOST", "https://api.example/v1")
	t.Setenv("SYNTHETIC_API_KEY", "k")
	t.Setenv("SYNTHETIC_DRY_RUN", "true")
	t.Setenv("SYNTHETIC_EVENT_INGEST_RATE", "12")
	t.Setenv("SYNTHETIC_LISTENER_PORT", "9000")
	t.Setenv("SYNTHETIC_CHECK_JANITOR_ENABLED", "false")
	t.Setenv("SYNTHETIC_CHECK_JANITOR_INTERVAL", "30m")
	c, err := LoadConfig()
	if err != nil {
		t.Fatalf("LoadConfig: %v", err)
	}
