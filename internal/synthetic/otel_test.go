...
func TestNewTracerProvider_DisabledReturnsNoop(t *testing.T) {
	tp, shutdown, err := NewTracerProvider(context.Background(), OTELConfig{Enabled: false}, "synthetic-test")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestNewTracerProvider_NoEndpoint_FallsBackToNoop(t *testing.T) {
	...
	tp, shutdown, err := NewTracerProvider(context.Background(), OTELConfig{Enabled: true}, "synthetic-test")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if tp == nil {
