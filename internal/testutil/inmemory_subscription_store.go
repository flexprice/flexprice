...
func subscriptionFilterFn(ctx context.Context, sub *subscription.Subscription, filter interface{}) bool {
	...
	// Exclude subscriptions whose parent_subscription_id is in this list
	if len(f.ExcludeParentSubscriptionIDs) > 0 {
		if sub.ParentSubscriptionID != nil && lo.Contains(f.ExcludeParentSubscriptionIDs, *sub.ParentSubscriptionID) {
			return false
		}
	}
...
func (s *InMemorySubscriptionStore) ListAll(ctx context.Context, filter *types.SubscriptionFilter) ([]*subscription.Subscription, error) {
	...
	unlimitedFilter := &types.SubscriptionFilter{
		...
		ExcludeParentSubscriptionIDs: filter.ExcludeParentSubscriptionIDs,
